...
//              
        ...
        function UpPageStat(){
            $now = get_rows("select DATE(current_timestamp) as dateNow");
            $date = get_rows("select MAX(DATE(p.created_dt)) as datePage from page_views p");
  ...
  /* function getDate(){
       $data = get_rows("select DATE(current_timestamp)");
      return $data;
   }
